...
app.post('/auth', async (req, res) => {
  try {
    ...
    // ❌ Nếu Apple trả lỗi thật sự (invalidcredentials)
    if (user.isBadLogin) {
      return res.json({
        success: false,
    ...
    // ✅ Nếu có dsid rõ → tài khoản không có 2FA
    if (user.dsid && user.dsid !== 'unknown') {
      return res.json({
        success: true,
        dsid: user.dsid
      });
    ...
    // ✅ Mọi trường hợp còn lại → cần xác minh (có thể đúng)
    return res.json({
      success: false,
      require2FA: true,
