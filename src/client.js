// client.js — FIX 2060: time headers động + đúng Content-Type cho từng bước
...
class Store {
  ...
  // Header động mỗi lần gọi (tránh lệch thời gian)
  ...
  static async purchase(adamId, Cookie) {
    ...
    const authHeaders = { 'X-Dsid': Cookie.dsPersonId, 'iCloud-DSID': Cookie.dsPersonId };
    ...
    if (!data.failureType && !data.dialog) return { ...data, _state: 'success' };

    // B2: follow dialog nhiều bước – ƯU TIÊN chính xác buyParams từ Apple
    ...
    if (data.failureType) {
      return { ...data, _state: 'failure', requiresAgeVerification: data.failureType === '2060' };
    }
...
// Giữ cho tương thích code cũ nếu chỗ nào lỡ dùng:
