...
export default async function handler(req, res) {
  ...
  try {
    ...
    // FIXED: Remove keychain dependency and use file-based auth
    const authConfigPath = path.join(tempDir, '.ipatool_config');
    ...
    const args = [
      ...
      // Remove keychain parameters completely
      '--non-interactive'  // Add non-interactive flag if available
    ];
    ...
    const ipatool = spawn('/usr/local/bin/ipatool', args, {
      env: {
        ...
        HOME: tempDir,
        TMPDIR: tempDir,
        // Set environment to prevent keychain access
        IPATOOL_KEYCHAIN_PASSPHRASE: '',
        // Try to force file-based storage
    ...
    // Check for keychain-related errors specifically
    if (errorLower.includes('keychain') || outputLower.includes('keychain') ||
        errorLower.includes('security framework') || outputLower.includes('security framework')) {
      statusCode = 500;
      errorType = 'KEYCHAIN_ERROR';
      errorMessage = 'Lỗi hệ thống bảo mật, đang thử phương pháp khác...';
